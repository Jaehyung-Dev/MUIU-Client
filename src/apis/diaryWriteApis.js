...
export const handleDiarySubmission = async (diaryData, userId, navigate) => {
  try {
    ...
    // 오늘 일기가 이미 작성되었는지 확인
    const checkResponse = await axios.get(`http://localhost:9090/diaries/check-today?userId=${userId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    ...
    if (checkResponse.data.exists) {
      ...
      console.log('Sending diary data:', diaryData);
      const writeResponse = await axios.post('http://localhost:9090/diaries/write', diaryData, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        },
